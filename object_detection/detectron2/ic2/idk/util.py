...
def build_icarl2_idk(cfg,num_oc,checkpoint=0):

    model = ICarl2RCNN(cfg)

    if checkpoint == 0:
        model.roi_heads = build_icarl2_heads(cfg,model.backbone.output_shape())
        DetectionCheckpointer(model).load(pretrained_fp)
    else:
        cfg.MODEL.ROI_HEADS.NUM_CLASSES = num_oc
        model.roi_heads = build_icarl2_heads(cfg,model.backbone.output_shape())
        wp = osp.join(loading_fp,str(checkpoint).zfill(5) + '_model.pth')
...
def update_idknetwork(cfg, model, num_oc, old_num):
    ...
    cfg.MODEL.ROI_HEADS.NUM_CLASSES = num_oc
    model.module.roi_heads =  build_icarl2_heads(cfg, model.module.backbone.output_shape())
    model.load_state_dict(w)
