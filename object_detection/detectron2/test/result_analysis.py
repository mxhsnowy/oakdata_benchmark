...
def get_cap_fix(all_predictions):
    ...
    for time_step in time_steps:
        ...
        for name in inter:
            ...
            outputs = predictions.get(str(val), [""])
            if outputs != [""]:
...
def get_cap_idk(all_predictions):
    ...
    # t_train_map = json.load(open(osp.join('/grogu/user/jianrenw/baseline/release/baseline_res/trainmap.json','r')))
    # trainmap_length = json.load(open(osp.join('/grogu/user/jianrenw/baseline/release/baseline_res/trainmap_length.json','r')))
    ...
    for time_step in time_steps:
        predictions = all_predictions[time_step]
        t_train_map = {} # # enter the corresponding train_map dir you saved
        train_map = {} 
...
f = open(osp.join(saved_dir,'res_' + method+'.json'),'w')
