...
def build_ewc_idk(cfg,num_oc,checkpoint=0):

    model = EWCRCNN(cfg)

    if checkpoint == 0:
        model.roi_heads = build_ewc_heads(cfg,model.backbone.output_shape())
...
def update_idknetwork(cfg, model, num_oc, old_num):
    ...
    cfg.MODEL.ROI_HEADS.NUM_CLASSES = num_oc
    model.module.roi_heads =  build_ewc_heads(cfg, model.module.backbone.output_shape())
    model.load_state_dict(w)
